...
var (
	defaultHomeDir        = bitumutil.AppDataDir("politeiavoter", false)
	defaultConfigFile     = filepath.Join(defaultHomeDir, defaultConfigFilename)
	defaultLogDir         = filepath.Join(defaultHomeDir, defaultLogDirname)
	bitumwalletHomeDir      = bitumutil.AppDataDir("bitumwallet", false)
	defaultWalletCertFile = filepath.Join(bitumwalletHomeDir, "rpc.cert")
)
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.PoliteiaWWW == "" {
		if activeNetParams.Name == "mainnet" {
			cfg.PoliteiaWWW = "https://proposals.bitum.io/api"
		} else {
			cfg.PoliteiaWWW = "https://test-proposals.bitum.io/api"
		}
	}
