...
func convertNewCommentFromBitum(nc bitumplugin.NewComment, ncr bitumplugin.NewCommentReply) Comment {
	return Comment{
...
func convertCommentFromBitum(c bitumplugin.Comment) Comment {
	return Comment{
...
func convertCommentToBitum(c Comment) bitumplugin.Comment {
	return bitumplugin.Comment{
		Token:       c.Token,
		ParentID:    c.ParentID,
		Comment:     c.Comment,
		Signature:   c.Signature,
...
func convertLikeCommentFromBitum(lc bitumplugin.LikeComment) LikeComment {
	return LikeComment{
		Token:     lc.Token,
...
func convertLikeCommentToBitum(lc LikeComment) bitumplugin.LikeComment {
...
func convertAuthorizeVoteFromBitum(av bitumplugin.AuthorizeVote, avr bitumplugin.AuthorizeVoteReply, version uint64) AuthorizeVote {
...
func convertAuthorizeVoteToBitum(av AuthorizeVote) bitumplugin.AuthorizeVote {
...
func convertStartVoteFromBitum(sv bitumplugin.StartVote, svr bitumplugin.StartVoteReply, endHeight uint64) StartVote {
...
func convertStartVoteToBitum(sv StartVote) (bitumplugin.StartVote, bitumplugin.StartVoteReply) {
	opts := make([]bitumplugin.VoteOption, 0, len(sv.Options))
	for _, v := range sv.Options {
		opts = append(opts, bitumplugin.VoteOption{
			Id:          v.ID,
			Description: v.Description,
			Bits:        v.Bits,
		})
	...
	dsv := bitumplugin.StartVote{
		PublicKey: sv.PublicKey,
		Signature: sv.Signature,
		Vote: bitumplugin.Vote{
			Token:            sv.Token,
			Mask:             sv.Mask,
	...
	dsvr := bitumplugin.StartVoteReply{
...
func convertCastVoteFromBitum(cv bitumplugin.CastVote) CastVote {
	return CastVote{
		Token:        cv.Token,
		Ticket:       cv.Ticket,
		VoteBit:      cv.VoteBit,
...
func convertCastVoteToBitum(cv CastVote) bitumplugin.CastVote {
...
func convertVoteOptionResultToBitum(r VoteOptionResult) bitumplugin.VoteOptionResult {
	return bitumplugin.VoteOptionResult{
		ID:          r.Option.ID,
...
func convertVoteOptionResultsToBitum(r []VoteOptionResult) []bitumplugin.VoteOptionResult {
	results := make([]bitumplugin.VoteOptionResult, 0, len(r))
	for _, v := range r {
		results = append(results, convertVoteOptionResultToBitum(v))
	}
	return results
}
