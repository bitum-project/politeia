...
import (
	...
	"github.com/bitum-project/politeia/politeiad/sharedconfig"
...
type config struct {
	...
	BitumtimeHost   string `long:"bitumtimehost" description:"Bitumtime ip:port"`
	BitumtimeCert   string `long:"bitumtimecert" description:"File containing the https certificate file for bitumtimehost"`
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.TestNet {
		var timeHost string
		if len(cfg.BitumtimeHost) == 0 {
			timeHost = v1.DefaultTestnetTimeHost
		} else {
			timeHost = cfg.BitumtimeHost
		}
		cfg.BitumtimeHost = util.NormalizeAddress(timeHost,
	...
	cfg.BitumtimeHost = "https://" + cfg.BitumtimeHost

	if len(cfg.BitumtimeCert) != 0 && !util.FileExists(cfg.BitumtimeCert) {
		cfg.BitumtimeCert = cleanAndExpandPath(cfg.BitumtimeCert)
		path := filepath.Join(cfg.HomeDir, cfg.BitumtimeCert)
		if !util.FileExists(path) {
			str := "%s: bitumtimecert " + cfg.BitumtimeCert + " and " +
				path + " don't exist"
			err := fmt.Errorf(str, funcName)
			fmt.Fprintln(os.Stderr, err)
			return nil, nil, err
