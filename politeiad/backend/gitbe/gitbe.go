...
type gitBackEnd struct {
	...
	bitumtimeHost     string           // Bitumtimed directory
...
// anchor takes a slice of commit digests and anchors them in bitumtime.
//
// This function is being clever with the anchors.  It sends two values to
// bitumtime.  We anchor the merkle root, and we *also* anchor all
...
// truly curious.  This is essentially free because bitumtime compresses all
// digests into a single merkle root.
//
// This function should be called with the lock held.
// TODO: the physical write to bitumtime needs to come out of the lock.
func (g *gitBackEnd) anchor(digests []*[sha256.Size]byte) error {
	...
	return util.Timestamp(g.bitumtimeHost, digests)
}
...
func (g *gitBackEnd) anchorRepo(path string) (*[sha256.Size]byte, error) {
	...
	// politeia's lookup key but bitumtime will likely return a different
	// merkle.  Bitumtime returns a different merkle when there are
...
// case it drops and anchor in bitumtime for each of them.
...
func (g *gitBackEnd) afterAnchorVerify(vrs []v1.VerifyDigest) error {
	...
	for _, vr := range vrs {
		if vr.ChainInformation.ChainTimestamp == 0 {
			// bitumtime returns 0 when there are not enough
			// confirmations yet.
		...
		// Mark test anchors as confirmed by bitumtime
		if g.test {
...
// verifyAnchor asks bitumtime if an anchor has been verified and returns a TX if
// it has.
func (g *gitBackEnd) verifyAnchor(digest string) (*v1.VerifyDigest, error) {
	...
	if g.test {
		...
		// Call bitumtime
		vr, err = util.Verify(g.bitumtimeHost, []string{digest})
		if err != nil {
			return nil, err
		}
	}
...
func (g *gitBackEnd) _updateRecord(commit bool, id string, mdAppend, mdOverwrite []backend.MetadataStream, fa []file, filesDel []string) error {
	...
	avFilename := fmt.Sprintf("%02v%v", bitumplugin.MDStreamAuthorizeVote,
		defaultMDFilenameSuffix)
	...
	// Call plugin hooks
	f, ok := bitumPluginHooks[PluginPostHookEdit]
	if ok {
...
// bitumtime.  This is an expensive operation and should not be run during
// runtime.
//
...
func (g *gitBackEnd) fsck(path string) error {
	...
	var seenAnchor bool
	// gitDigests is an index of all git digests to verify with bitumtime
	gitDigests := make(map[string]struct{})
	// confirmedAnchors keeps track of anchors that were timestamped with bitumtime but not verified,
	...
	for _, v := range out {
		if regexAnchorConfirmation.MatchString(v) {
			...
			// If we didn't, save the anchor key to verify with bitumtime later
	...
	log.Infof("fsck: bitumtime verification started")
	...
	vr, err := util.Verify(g.bitumtimeHost, digests)
	if err != nil {
		return err
	}
	...
	for _, v := range vr.Digests {
		if v.Result != v1.ResultOK {
			fail = true
			log.Errorf("bitumtime error: %v %v %v", v.Digest,
				v.Result, v1.Result[v.Result])
		}
	}
	if fail {
		return fmt.Errorf("bitumtime fsck failed")
	}
...
func New(anp *chaincfg.Params, root string, bitumtimeHost string, gitPath string, id *identity.FullIdentity, gitTrace bool) (*gitBackEnd, error) {
	...
	g := &gitBackEnd{
		...
		gitPath:         gitPath,
		bitumtimeHost:     bitumtimeHost,
		gitTrace:        gitTrace,
		exit:            make(chan struct{}),
		checkAnchor:     make(chan struct{}),
		testAnchors:     make(map[string]bool),
		plugins:         []backend.Plugin{getBitumPlugin(anp.Name != "mainnet")},
	}
	idJSON, err := id.Marshal()
	if err != nil {
		return nil, err
	}
	setBitumPluginSetting(bitumPluginIdentity, string(idJSON))
	setBitumPluginSetting(bitumPluginJournals, g.journals)
	setBitumPluginHook(PluginPostHookEdit, g.bitumPluginPostEdit)
	...
	// Message user
	log.Infof("Timestamp host: %v", g.bitumtimeHost)

	log.Infof("Running bitumtime fsck on vetted repository")
	err = g.fsck(g.vetted)
	if err != nil {
		// Log error but continue
		log.Errorf("fsck: bitumtime %v", err)
	}
