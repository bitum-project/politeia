...
import (
	...
	"github.com/bitum-project/politeia/politeiad/backend/gitbe"
	"github.com/bitum-project/politeia/politeiad/cache"
	"github.com/bitum-project/politeia/politeiad/cache/cachestub"
	"github.com/bitum-project/politeia/politeiad/cache/cockroachdb"
...
func _main() error {
	...
	if len(loadedCfg.BitumtimeCert) != 0 {
		var certPool *x509.CertPool
		if !util.FileExists(loadedCfg.BitumtimeCert) {
			return fmt.Errorf("unable to find bitumtime cert %v",
				loadedCfg.BitumtimeCert)
		}
		bitumtimeCert, err := ioutil.ReadFile(loadedCfg.BitumtimeCert)
		if err != nil {
			return fmt.Errorf("unable to read bitumtime cert %v: %v",
				loadedCfg.BitumtimeCert, err)
		}
		certPool = x509.NewCertPool()
		if !certPool.AppendCertsFromPEM(bitumtimeCert) {
	...
	b, err := gitbe.New(activeNetParams.Params, loadedCfg.DataDir,
		loadedCfg.BitumtimeHost, "", p.identity, loadedCfg.GitTrace)
	if err != nil {
		return err
	}
	p.backend = b
