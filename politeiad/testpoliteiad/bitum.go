...
import (
	...
	v1 "github.com/bitum-project/politeia/politeiad/api/v1"
)
...
func (p *TestPoliteiad) authorizeVote(payload string) (string, error) {
	av, err := bitum.DecodeAuthorizeVote([]byte(payload))
	if err != nil {
		return "", err
	}
	...
	av.Version = bitum.VersionAuthorizeVote
	...
	if !ok {
		p.authorizeVotes[av.Token] = make(map[string]bitum.AuthorizeVote)
	}
	...
	// Prepare reply
	avrb, err := bitum.EncodeAuthorizeVoteReply(
		bitum.AuthorizeVoteReply{
			Action:        av.Action,
...
func (p *TestPoliteiad) startVote(payload string) (string, error) {
	sv, err := bitum.DecodeStartVote([]byte(payload))
	if err != nil {
		return "", err
	}
	...
	svr := bitum.StartVoteReply{
		Version:          bitum.VersionStartVoteReply,
	...
	svrb, err := bitum.EncodeStartVoteReply(svr)
	if err != nil {
		return "", err
	}
...
// bitumExec executes the passed in plugin command.
func (p *TestPoliteiad) bitumExec(pc v1.PluginCommand) (string, error) {
	switch pc.Command {
