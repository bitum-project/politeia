...
const (
	...
	// Note that 14 is in use by the bitum plugin
	// Note that 15 is in use by the bitum plugin
...
func voteStatusFromVoteSummary(r bitumplugin.VoteSummaryReply, bestBlock uint64) www.PropVoteStatusT {
	switch {
	case !r.Authorized:
		return www.PropVoteStatusNotAuthorized
	case r.EndHeight == "":
...
func (p *politeiawww) getProp(token string) (*www.ProposalRecord, error) {
	...
	dc, err := p.bitumGetComments(token)
	if err != nil {
		log.Errorf("getProp: bitumGetComments failed "+
			"for token %v", token)
	}
...
func (p *politeiawww) getPropVersion(token, version string) (*www.ProposalRecord, error) {
	...
	dc, err := p.bitumGetComments(token)
	if err != nil {
		log.Errorf("getPropVersion: bitumGetComments "+
...
func (p *politeiawww) getAllProps() ([]www.ProposalRecord, error) {
	...
	for _, v := range records {
		...
		dc, err := p.bitumGetComments(pr.CensorshipRecord.Token)
		if err != nil {
			log.Errorf("getAllProps: bitumGetComments failed "+
...
func (p *politeiawww) getPropComments(token string) ([]www.Comment, error) {
	...
	dc, err := p.bitumGetComments(token)
	if err != nil {
		return nil, fmt.Errorf("bitumGetComments: %v", err)
	}
...
func (p *politeiawww) processSetProposalStatus(sps www.SetProposalStatus, u *user.User) (*www.SetProposalStatusReply, error) {
	...
	case pr.State == www.PropStateVetted:
		...
		vdr, err := p.bitumVoteDetails(pr.CensorshipRecord.Token)
		if err != nil {
			return nil, fmt.Errorf("bitumVoteDetails: %v", err)
		}
		vd := convertVoteDetailsReplyFromBitum(*vdr)
...
func (p *politeiawww) processEditProposal(ep www.EditProposal, u *user.User) (*www.EditProposalReply, error) {
	...
	vdr, err := p.bitumVoteDetails(ep.Token)
	if err != nil {
		return nil, fmt.Errorf("bitumVoteDetails: %v", err)
	}
	vd := convertVoteDetailsReplyFromBitum(*vdr)
...
func (p *politeiawww) voteStatusReply(token string, bestBlock uint64) (*www.VoteStatusReply, error) {
	...
	r, err := p.bitumVoteSummary(token)
	if err != nil {
		return nil, err
	}

	results := convertVoteOptionResultsFromBitum(r.Results)
	var total uint64
...
func (p *politeiawww) processActiveVote() (*www.ActiveVoteReply, error) {
	...
	for _, v := range all {
		...
		vdr, err := p.bitumVoteDetails(v.CensorshipRecord.Token)
		if err != nil {
			log.Errorf("processActiveVote: bitumVoteDetails failed %v: %v",
...
func (p *politeiawww) processVoteResults(token string) (*www.VoteResultsReply, error) {
	...
	// Get vote details from cache
	vdr, err := p.bitumVoteDetails(token)
	if err != nil {
		return nil, fmt.Errorf("bitumVoteDetails: %v", err)
	}

	// Get cast votes from cache
	vrr, err := p.bitumProposalVotes(token)
	if err != nil {
		return nil, fmt.Errorf("bitumVoteDetails: %v", err)
	}
	...
	return &www.VoteResultsReply{
		...
		CastVotes:      convertCastVotesFromBitum(vrr.CastVotes),
	}, nil
}
...
func (p *politeiawww) processCastVotes(ballot *www.Ballot) (*www.BallotReply, error) {
	...
	payload, err := bitumplugin.EncodeBallot(convertBallotFromWWW(*ballot))
	if err != nil {
		return nil, err
	}
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        bitumplugin.ID,
		Command:   bitumplugin.CmdBallot,
		CommandID: bitumplugin.CmdBallot,
	...
	// Decode plugin reply
	br, err := bitumplugin.DecodeBallotReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
	brr := convertBallotReplyFromBitumPlugin(*br)
	return &brr, nil
}
...
// processAuthorizeVote sends the authorizevote command to bitum plugin to
...
func (p *politeiawww) processAuthorizeVote(av www.AuthorizeVote, u *user.User) (*www.AuthorizeVoteReply, error) {
	...
	// Get vote details from cache
	vdr, err := p.bitumVoteDetails(av.Token)
	if err != nil {
		return nil, fmt.Errorf("bitumVoteDetails: %v", err)
	}
	vd := convertVoteDetailsReplyFromBitum(*vdr)
	...
	payload, err := bitumplugin.EncodeAuthorizeVote(dav)
	if err != nil {
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        bitumplugin.ID,
		Command:   bitumplugin.CmdAuthorizeVote,
		CommandID: bitumplugin.CmdAuthorizeVote + " " + av.Token,
		Payload:   string(payload),
	}
	...
	// Decode plugin reply
	avr, err := bitumplugin.DecodeAuthorizeVoteReply([]byte(reply.Payload))
	if err != nil {
...
func (p *politeiawww) processStartVote(sv www.StartVote, u *user.User) (*www.StartVoteReply, error) {
	...
	payload, err := bitumplugin.EncodeStartVote(dsv)
	if err != nil {
		return nil, err
	}
	...
	// Get vote details from cache
	vdr, err := p.bitumVoteDetails(sv.Vote.Token)
	if err != nil {
		return nil, fmt.Errorf("bitumVoteDetails: %v", err)
	}
	vd := convertVoteDetailsReplyFromBitum(*vdr)
	...
	// Tell bitum plugin to start voting
	challenge, err := util.Random(pd.ChallengeSize)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        bitumplugin.ID,
		Command:   bitumplugin.CmdStartVote,
		CommandID: bitumplugin.CmdStartVote + " " + sv.Vote.Token,
		Payload:   string(payload),
	}
	...
	vr, err := bitumplugin.DecodeStartVoteReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
	...
	// return a copy
	rv := convertStartVoteReplyFromBitum(*vr)
	return &rv, nil
}
...
func (p *politeiawww) processTokenInventory() (*www.TokenInventoryReply, error) {
	...
	for retries := 0; !done && retries <= 1; retries++ {
		...
		r = convertTokenInventoryReplyFromBitum(*ti)
		done = true
	}
