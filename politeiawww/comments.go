...
func (p *politeiawww) initCommentScores() error {
	...
	// Fetch bitum plugin inventory from cache
	ir, err := p.bitumInventory()
	if err != nil {
		return fmt.Errorf("bitumInventory: %v", err)
	}
...
func (p *politeiawww) getComment(token, commentID string) (*www.Comment, error) {
	...
	dc, err := p.bitumGetComment(token, commentID)
	if err != nil {
		return nil, fmt.Errorf("bitumGetComment: %v", err)
	}
	c := convertCommentFromBitum(*dc)
...
func (p *politeiawww) updateCommentScore(token, commentID string) (int64, error) {
	...
	likes, err := p.bitumCommentLikes(token, commentID)
	if err != nil {
		return 0, fmt.Errorf("bitumLikeComments: %v", err)
	}
...
// processNewComment sends a new comment bitum plugin command to politeaid
...
func (p *politeiawww) processNewComment(nc www.NewComment, u *user.User) (*www.NewCommentReply, error) {
	...
	vdr, err := p.bitumVoteDetails(nc.Token)
	if err != nil {
		return nil, fmt.Errorf("bitumVoteDetails: %v", err)
	}
	_, avr := convertAuthVoteFromBitum(vdr.AuthorizeVote)
	svr := convertStartVoteReplyFromBitum(vdr.StartVoteReply)
	...
	dnc := convertNewCommentToBitumPlugin(nc)
	payload, err := bitumplugin.EncodeNewComment(dnc)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		...
		ID:        bitumplugin.ID,
		Command:   bitumplugin.CmdNewComment,
		CommandID: bitumplugin.CmdNewComment,
	...
	ncr, err := bitumplugin.DecodeNewCommentReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
...
// processNewCommentInvoice sends a new comment bitum plugin command to politeaid
// then fetches the new comment from the cache and returns it.
...
func (p *politeiawww) processLikeComment(lc www.LikeComment, u *user.User) (*www.LikeCommentReply, error) {
	...
	// Ensure proposal voting has not ended
	vdr, err := p.bitumVoteDetails(lc.Token)
	if err != nil {
		return nil, fmt.Errorf("bitumVoteDetails: %v", err)
	}
	...
	_, err = p.bitumGetComment(lc.Token, lc.CommentID)
	if err != nil {
		if err == cache.ErrRecordNotFound {
			err = www.UserError{
	...
	dlc := convertLikeCommentToBitum(lc)
	payload, err := bitumplugin.EncodeLikeComment(dlc)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        bitumplugin.ID,
		Command:   bitumplugin.CmdLikeComment,
		CommandID: bitumplugin.CmdLikeComment,
		Payload:   string(payload),
	}
	...
	lcr, err := bitumplugin.DecodeLikeCommentReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
...
// processCensorComment sends a censor comment bitum plugin command to
...
func (p *politeiawww) processCensorComment(cc www.CensorComment, u *user.User) (*www.CensorCommentReply, error) {
	...
	c, err := p.bitumGetComment(cc.Token, cc.CommentID)
	if err != nil {
		return nil, fmt.Errorf("bitumGetComment: %v", err)
	}
	if c.Censored {
	...
	// Ensure proposal voting has not ended
	vdr, err := p.bitumVoteDetails(cc.Token)
	if err != nil {
		return nil, fmt.Errorf("bitumVoteDetails: %v", err)
	}
	vd := convertVoteDetailsReplyFromBitum(*vdr)
	...
	dcc := convertCensorCommentToBitum(cc)
	payload, err := bitumplugin.EncodeCensorComment(dcc)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        bitumplugin.ID,
		Command:   bitumplugin.CmdCensorComment,
		CommandID: bitumplugin.CmdCensorComment,
		Payload:   string(payload),
	}
	...
	ccr, err := bitumplugin.DecodeCensorCommentReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
