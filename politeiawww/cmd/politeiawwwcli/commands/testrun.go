...
import (
	...
	"github.com/bitum-project/bitumwallet/rpc/walletrpc"
...
func (cmd *TestRunCmd) Execute(args []string) error {
	...
	if paywallEnabled {
		...
		bitum := float64(lr.PaywallAmount) / 1e8
		fmt.Printf("  Paid %v BITUM to %v with txID %v\n",
			bitum, lr.PaywallAddress, txID)
	}
	...
	if err != nil {
		...
		case strings.Contains(err.Error(), "connection refused"):
			// User is not running a bitumwallet instance locally.
			// This is ok. Print a warning and continue.
			fmt.Printf("  WARNING: could not connect to bitumwallet; " +
				"skipping cast votes\n")
			skipCastVotes = true
...
Paywall: 
If the politeiawww paywall is enabled the test run will use the Bitum tesnet
...
Voting:
The test run will attempt to vote on a proposal.  If a bitumwallet instance is
