...
`https://proposals.bitum.io`.  Copy these lines into your
...
`--paywall` will use the Bitum testnet faucet to satisfy the user registration
fee requirement. 
...
prevention measure that would normally need to be purchased using BITUM, but if
...
$ politeiawww_dbutil -testnet -adbitumedits email@example.com 50
```
...
[politeiavoter](https://github.com/bitum-project/politeia/tree/master/politeiavoter/)
tool.

### Voting on a proposal - politeiawwwcli
You can also vote on proposals using `politeiawwwcli`, but for right now, it 
only works on testnet and you have to be running your bitumwallet locally using
